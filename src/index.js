...
const {checkWalletBalance, airdropSOL, transferSOL,}=require("./solana");
const { getReturnAmount, totalAmtToBePaid, randomNumber } = require('./helper');



// Create a prize pool wallet
const prizeWallet = web3.Keypair.generate();
const prizePubKey = new web3.PublicKey(prizeWallet._keypair.publicKey);
const prizeSecKey = prizeWallet.secretKey;
...
const userPubkey = new web3.PublicKey(userWallet.publicKey.toString())
console.log(`user public key is ${userPubkey} `)
...
const treasuryPubkey = new web3.PublicKey(treasuryWallet._keypair.publicKey)

const askQuestions = () => {
    const questions = [
        {
            // Ask how much SOL to bet(stake) with
            name: "SOL",
            type: "number",
            message: "What is the amount of SOL you want to stake?",
        },
        {
            // Ask for staking ratio
            type: "rawlist",
            name: "RATIO",
        ...
        },
        {
            ...
            when:async (val)=>{
                if(parseFloat(totalAmtToBePaid(val.SOL))>5){
                    ...
                    const userBalance=await checkWalletBalance(connection, userWallet.publicKey)
...
const gameExecution=async ()=>{
    ...
    if(answers.RANDOM){
        const paymentSignature=await transferSOL(
            userWallet,treasuryWallet,totalAmtToBePaid(answers.SOL))
        ...
        if(answers.RANDOM===generateRandomNumber){
            ...
            await airdropSOL(
               connection, treasuryPubkey ,getReturnAmount(answers.SOL,parseFloat(answers.RATIO)));
